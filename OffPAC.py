...
class ActorNet(nn.Module):      # 定义Actor网络
...
class ActorCritic(object):
    ...
    def choose_action(self, s):  # normal-distribution
        ...
        action = np.clip(np.random.normal(self.actor_target(s)[0].detach().cpu(), np.sqrt(np.exp(self.actor_target(s)[1].detach().cpu()))),
                         a_low_bound, a_bound)
        return action

    def learn(self, sample):
        # 采用Q-learning的方式来更新权值
        ...
        # train actor network
        miu, rou = self.actor_target(state)     # 得到高斯分布的两个参数
        # a = np.clip(np.random.normal(miu, np.exp(rou)), a_low_bound, a_bound)   # sample
        ln_pdf = torch.log(torch.normal(miu, torch.sqrt(torch.exp(rou))))      # pdf
...
# for i in range(EPISODE):
    ...
    for j in range(EP_STEPS):
        ...
        state = next_state
        step += 1
        if step % 5 == 0:   # 每过5个episode就更新target网络
            Ac.actor_target.load_state_dict(Ac.actor.state_dict())
    # f = open('reward_OffPAC_' + ENV_NAME + '.txt', 'a+')
